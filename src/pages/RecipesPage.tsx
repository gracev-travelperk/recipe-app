...
export default function RecipesPage() {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSearchTerm(query);
    const searchedRecipes = recipeIdeas.filter((recipe: Recipe) =>
  ...
  const deleteRecipe = (targetId: number) => {
    const newRecipes = recipeIdeas.filter(
      (recipe: Recipe) => recipe.id !== targetId
    );
    setRecipeIdeas(newRecipes);
    setFilteredRecipes(
      newRecipes.filter((recipe: Recipe) =>
        recipe.name.toLowerCase().includes(searchTerm.toLowerCase())
      )
    );
  };
  const updateRecipe = (id: number, updatedRecipe: Recipe) => {
    const newRecipes = recipeIdeas.map((recipe: Recipe) =>
